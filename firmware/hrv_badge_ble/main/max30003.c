...
void max30003_initchip(int pin_miso, int pin_mosi, int pin_sck, int pin_cs )
{
  ...
  spi_device_interface_config_t devcfg=
  {
      ...
      .spics_io_num=pin_cs,              		
      .queue_size=7,                          
      .pre_cb=max30003_spi_pre_transfer_callback,  
  };
  //Initialize the SPI bus
  ret=spi_bus_initialize(HSPI_HOST, &buscfg, 0);					//use 1 to enable DMA, should use idf version 3.0/2.0
  assert(ret==ESP_OK);
...
{	
  //options : ANIMATION, INTENSITY
  blink_heart(INTENSITY);
  //blink_heart(ANIMATION);
  update_beat_blr();
